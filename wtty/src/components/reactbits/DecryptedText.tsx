...
export default function DecryptedText({
  ...
  const shuffleText = useCallback(
    (originalText: string, currentRevealed: Set<number>) => {
      ...
      return safeText
        ...
        .map((char, i) => {
          if (char === ' ') return ' ';
          if (currentRevealed.has(i)) return safeText[i];
  ...
  const fillAllIndices = useCallback(() => {
    ...
    for (let i = 0; i < (text?.length || 0); i++) s.add(i);
    return s;
  }, [text]);
  ...
  const triggerDecrypt = useCallback(() => {
    if (sequential) {
      orderRef.current = computeOrder(text?.length || 0);
  ...
  const triggerReverse = useCallback(() => {
    ...
    if (sequential) {
      orderRef.current = computeOrder(len).slice().reverse();
      pointerRef.current = 0;
      ...
      setRevealedIndices(prevRevealed => {
        ...
        if (sequential) {
          ...
          if (direction === 'reverse') {
            ...
            const removeCount = Math.max(1, Math.ceil(len / Math.max(1, maxIterations)));
