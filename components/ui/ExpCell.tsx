...
export function ExpCell({ id, entity, role, desc, icon: Icon, isDark }: ExpCellProps) {
  return (
    <div className={`p-[var(--spacing-section)] border-t border-muted group hover:bg-surface-sunken transition-colors flex flex-col h-full ${isDark ? "bg-black text-white" : "bg-white text-black"}`}>
      <div className="flex justify-between items-start mb-8">
        <span className="font-mono text-[9px] opacity-40 tracking-[0.4em] uppercase font-bold">{id} // LOG_DE_EMPRESAS</span>
        {Icon && <Icon size={14} aria-hidden="true" className={`opacity-20 group-hover:opacity-100 transition-opacity ${isDark ? "text-white" : "text-ink"}`} strokeWidth={1.5} />}
      </div>
